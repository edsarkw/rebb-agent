...
async function createTask(fields) {
  const records = await base()(TABLES.tasks).create([{ fields }]);
...
async function updateTask(id, fields) {
  const rec = await base()(TABLES.tasks).update(id, fields);
  return { id: rec.id, fields: rec.fields };
}
...
async function atomicFlipDraftToSent(id, expectedJti) {
  ...
  if (status === 'Sent') return { ok: false, reason: 'already_sent', task };
  if (status === 'Failed') return { ok: false, reason: 'already_failed', task };
  if (status !== 'Draft') return { ok: false, reason: `bad_status_${status}`, task };
